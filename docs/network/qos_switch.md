 ...
 * Strict-Priority. Это обычно одна очередь (8ая), откуда данные передаются в первую очередь.
...
По умолчанию не все коммутаторы анализируют dscp в ip-заголовках и настроены на работу с полем cos. Например, у juniper ex
```text
...
Весь трафик перемаркировывается, ему присваивается соответствующая qos-group'а, у которой приоритеты определены процентами от полосы.
```yaml
---
